<template>
  <div class="relative min-h-screen w-full bg-[#1C1F24]/75" tabindex="0" @keydown="handleKeyDown" @wheel="handleWheel" >
    <!-- 场景栏 -->
    <div class="w-full bg-[#181A1E]/90 border-b border-gray-200/65 h-10 relative">
      <div class="flex items-center space-x-1 px-2">
        <div v-for="(tab, index) in tabs" :key="index"
          class="px-4 py-2 cursor-pointer rounded-t-lg flex items-center gap-2" :class="{
            'bg-[#707277] border-b-2 border-blue-500': activeTab === index,
            'hover:bg-gray-200/65': activeTab !== index
          }" @click="switchTab(index)" @dblclick="openSceneBar(index)">
          {{ tab.name }}
          <button v-if="tabs.length > 1" @click.stop="closeTab(index)" class="hover:bg-[#374151] rounded-full p-1">
            ×
          </button>
        </div>

        <button @click="addNewTab" class="px-3 py-1 text-xl font-bold hover:bg-[#374151] rounded-lg">
          +
        </button>
...
const cabbagetalk = () => {
  ...
  if (window.pyBridge) {
    window.pyBridge.addDockWidget("PetPE", "/PetPE", "float", "bottom_right", JSON.stringify(size));
  }
};
...
const openSceneBar = (index) => {
  if (window.pyBridge) {
    ...
    window.pyBridge.addDockWidget(sceneName, `/SceneBarPE?sceneName=${sceneName}`, "left");
  } else {
