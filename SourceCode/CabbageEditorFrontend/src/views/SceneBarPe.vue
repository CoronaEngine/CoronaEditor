<template>
  <div class="border-2 border-[#84a65b] rounded-md relative bg-[#1C1F24]/75">
    <!-- 标题栏 -->
    <div class="titlebar flex items-center w-full p-2 rounded-t-md justify-between bg-[#181A1E] cursor-move select-none"
      ...
      <div class="flex w-full space-x-2 justify-end">
        <button @click.stop="closeFloat"
          class="px-2 py-1 bg-gray-700 hover:bg-[#374151] text-white text-sm rounded transition-colors duration-200">
          ×
        </button>
      </div>
    </div>
    ...
    <div class="p-4 shadow-md w-full bg-[#a8a4a3]/65 flex flex-col" style="height: calc(100vh - 56px);">
      <div class="relative mb-4">
        <button @click.stop="handleFileImport"
          class="mr-4 px-2 py-1 bg-gray-700 hover:bg-[#707277]/0 text-white text-sm rounded transition-colors duration-200">
          导入模型
        </button>
      ...
      <div class="flex items-center justify-between space-x-4 mb-4">
        ...
        <input type="number" step="0.1" @change="updateSunPosition" @input="e => px = e.target.value"
          class="w-20 p-1 text-center border rounded-md focus:outline-none focus:ring-2 text-write focus:ring-blue-400 bg-[#707277]/0"
          :value="px" />
